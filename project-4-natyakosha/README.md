...
## How to Use Natyakosha

Once the app is running and seeded, log in at [http://localhost:5173/](http://localhost:5173/) with one of the seeded accounts:

| Role       | Username                              | Password     |
| ---------- | ------------------------------------- | ------------ |
| Instructor | `neeraja`                             | `teacher123` |
| Student    | `keshvi` (or `priya`, `ananya`, etc.) | `student123` |

**As a student:**

1. Log in and land on the **Learning Area**. Switch between the **Theory & Texts**, **Mudra Gallery**, and **Adavu Steps** tabs to browse curriculum content.
2. Click any Theory card to expand it and read the full text; click again to collapse it.
3. Open **Records** in the top nav to see your attendance presence-rate ring, class history, and fee ledger. Click **Pay Now** next to any outstanding fee cycle to mark it paid.
4. Use the **Account Details** tab under Records to change your password.

**As the instructor:**

1. Log in and land on the **Curriculum Editor**. Fill out the form to add a new Theory/Mudra/Adavu item, or edit/delete an existing item from the list below the form.
2. Open **Roster Manager** to register a new student (a username is auto-suggested from the first/last name you type) and assign them to one of the four evening batches. Remove a student from a batch with the ✕ button next to their name.
3. Open **Mark Attendance**, pick a batch and date, and mark each student present or absent with the radio controls, then **Save Register Entries**.
4. Open **Fee Dashboard** to see total collected, total outstanding, and unpaid invoices across all batches. Filter by batch timing, and click **Mark Paid** on any unpaid row to record a manual payment.

---

## Code Quality & Formatting

Linting rules are managed via `eslint` and `oxlint`, and code formatting is enforced via `prettier`.

```bash
# Navigate to the frontend directory
cd frontend

# Run both linters (oxlint, then eslint)
npm run lint
```

To check or apply formatting across the whole project, run from the root folder:
